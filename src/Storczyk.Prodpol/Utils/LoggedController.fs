...
open Storczyk.Prodpo.Utils
open Storczyk.Prodpo.Utils.AsyncResult
...
type LoggedController() =
    ...
    member this.ValidateObject (obj: 'a): Result<'a, DatabaseError> =
        if this.TryValidateModel(obj) then
            ...
            Error (DatabaseError.ValidationErrors (this.ModelState
                    |> Seq.map (fun kv -> { Field = kv.Key; Issue = kv.Value.AttemptedValue })))
