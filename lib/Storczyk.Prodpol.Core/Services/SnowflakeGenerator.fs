...
module Snowflake =
    ...
    let DefaultSnowflakeOptions: SnowflakeOptions =
        { Epoch = DateTimeOffset.FromUnixTimeSeconds(1778882400L).DateTime; InstanceId = 1uL }
...
type SnowflakeGenerator(options: SnowflakeOptions) =
    ...
    interface ISnowflakeGenerator with
        member this.GetSnowflake() = (this :> ISnowflakeGenerator).GetSnowflake(DateTime.UtcNow)
